...
const AdmittedDialog = ({
  setShowAdmittedDialog,
  statusList,
  currentModification,
  fetchAllTransaction,
  ...
  const handleSubmit = () => {
    if (admittedData.datePerformed === "") {
      setSnackbarOpen({
        open: true,
        message: "ADMISSION DATE NOT FOUND",
        type: "warning",
      });
    } else if (admittedData.weight === "") {
      setSnackbarOpen({
        open: true,
        message: "Weight not found",
        type: "warning",
      });
    } else if (admittedData.internalPerson === "") {
      setSnackbarOpen({
        open: true,
        message: "No Internal person is assigned",
        type: "warning",
      });
    } else if (!imgSrc) {
      setSnackbarOpen({
        open: true,
        message: "ADMISSION IMAGE NOT FOUND",
        type: "warning",
      });
    } else {
      setLoading(true);
      const data = {
        AdmittedDate: admittedData.datePerformed,
        AdmittedImg: imgSrc,
        Weight: admittedData.weight,
        StatusId: statusList.filter(
          (status) => status.StatusName === "admitted"
        )[0].id,
        AssignedPersonInternal: admittedData.internalPerson,
      };
      console.log(data);
      axios
        .put(
          `${import.meta.env.VITE_API_URL}/transaction/catUpdate/admit/${
        ...
        <div className="bg-white rounded-xl w-[95%] lg:w-[60%] flex flex-col justify-between items-center max-h-full overflow-y-auto">
          ...
          <div className="w-full px-10 flex flex-col justify-center items-center gap-3 py-8">
            ...
            <CustomInputField
              ...
              type="date"
              id="admittedDate"
              value={admittedData.datePerformed}
              onChange={(e) =>
                setAdmittedData({
                  ...admittedData,
              ...
              InputProps={{
                endAdornment: (
                  <InputAdornment position="end">
                    <span className="font-medium text-xl text-grey-900">
                      kg
                    </span>
                  </InputAdornment>
                ),
              }}
              value={admittedData.weight}
              onChange={(e) =>
                setAdmittedData({
                  ...admittedData,
            ...
            />
            <CustomSelectField
              label="Assign Person Internal"
              labelSelector="userName"
              id="internalPerson"
              options={internalUserList.sort((a, b) => {
              ...
              value={admittedData.internalPerson}
              onChange={(e) =>
                setAdmittedData({
                  ...admittedData,
                  internalPerson: e.target.value,
                })
              }
            />
            ...
            <div className="flex w-full my-3 items-center justify-center gap-5">
              <div
                ...
                onClick={() => {
                  setShowAdmittedDialog((prev) => !prev);
                }}
              >
                Cancel
              </div>
