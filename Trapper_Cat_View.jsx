  ...
  const fetchAllTransaction = () => {
    ...
    const url = JSON.parse(localStorage.getItem("AFA_user"))?.role.includes(
      ...
      ? `${
        ...
        }/api/v2/transaction/sterilization/cat/allTrappedCases`
      : `${
          import.meta.env.VITE_API_URL
          ...
          setTrapsBackup(
            sortArrayByUpdatedAt(
              refactoredResponse.filter(
                (trap) =>
                  trap.StatusName === "surgery" || trap.StatusName === "trapped"
              ),
              isDescending
            )
          );
...
export default Trapper_Cat_View;
