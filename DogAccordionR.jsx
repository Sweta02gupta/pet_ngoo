...
const DogAccordionR = ({
  ...
  const handleSaveWeight = () => {
    if (weight.length) {
      ...
      axios
        .put(
          `${import.meta.env.VITE_API_URL}/api/v2/rescue/dog/update/${trap.id}`,
          JSON.stringify(payload),
          {
            headers: {
              Authorization: `Bearer ${JSON.parse(localStorage.getItem("AFA_user")).token
                }`,
        ...
        <div className="flex items-center gap-2">
          {trap.StatusName === "released" ? (
            <>
              {fetchImage ? (
                <Avatar
                  src={`${import.meta.env.VITE_API_URL}/image/${trap.ReleaseImg
                    }`}
                  alt="dog"
                />
              ) : (
          ...
          {trap.StatusName === "released" && (
            <span className="text-sm text-green-500 font-bold border-2 border-green-500 rounded-full text-center p-1 bg-green-50">
              RELEASED
            </span>
          )}
          {trap?.Comment && trap.Comment.length > 0 && (
            <span className="border rounded-full bg-blue-600">
              <BsInfo color="#fff" size={20} />
            </span>
          )}
        </div>
      </AccordionSummary>
        ...
        <div className="flex flex-col justify-center items-center gap-4 w-full px-4 md:px-8 lg:px-16 xl:px-28 text-sm md:text-base">
          ...
          {trap.SurgeryDate && (
            <div className="w-full flex flex-col justify-center items-center gap-2">
              ...
              <ImageWithZoom
                src={`${import.meta.env.VITE_API_URL}/image/${trap.TreatmentImg
                  }`}
                alt="cat"
              />
            </div>
              ...
              <div className="w-full flex flex-wrap items-center justify-start gap-2">
                {!trap.SurgeryDate && (
                  <div className="flex items-center justify-center gap-2 text-red-600 border border-red-600 py-1 px-2 rounded-full bg-red-50">
                    Treatment <AiFillCloseCircle className="text-[15px]" />
                  </div>
                )}
                {!trap.ReleaseDate && (
                  <div className="flex items-center justify-center gap-2 text-red-600 border border-red-600 py-1 px-2 rounded-full bg-red-50">
