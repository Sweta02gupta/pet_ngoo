...
const ReleaseDialogCat = ({
      ...
      if (modifierType === "trapper") {
        ...
        axios
          .put(
            ...
            }/api/v2/transaction/sterilization/cat/update/release/${
        ...
        axios
          .put(
            `${
              import.meta.env.VITE_API_URL
            }/api/v2/transaction/sterilization/cat/update/transactionStatus/${
              currentModification.id
            }`,
            JSON.stringify(data),
            {
  ...
  const filterStatuses = () => {
    ...
    // Convert to array of objects for filtering
    return statusOptions[currentStatus]?.map((status) => ({
      id: status.toLowerCase(), // Assuming string as id for simplicity
      StatusName: status,
    })) || [];
  };
              ...
              {modifierType === "admin" ? (
                ...
                >
                  {filterStatuses()
                    .filter(
                      (status) =>
                        status.id !== currentModification.StatusId &&
                        status.StatusName.toLowerCase() !== "trapped" &&
                        !(
                          currentModification.StatusName.toLowerCase() === "trapped" &&
                          status.StatusName.toLowerCase() === "release without surgery"
                        )
                    )
...
export default ReleaseDialogCat;
