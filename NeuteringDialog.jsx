...
const NeuteringDialog = ({
  setShowNeuteringDialog,
  statusList,
  currentModification,
  fetchAllTransaction,
  ...
  const handleSubmit = () => {
    if (neuteringData.datePerformed === "") {
      setSnackbarOpen({
        open: true,
        message: "NEUTERING DATE NOT FOUND",
        type: "warning",
      });
    } else if (!imgSrc) {
      setSnackbarOpen({
        open: true,
        message: "NEUTERING IMAGE NOT FOUND",
        type: "warning",
      });
    } else {
      setLoading(true);
      const data = {
        NeutringDate: neuteringData.datePerformed,
        NeuteringImg: imgSrc,
        StatusId: statusList.filter(
          (status) => status.StatusName === "neutering"
        )[0].id,
      };
      console.log(data);
      axios
        .put(
          `${import.meta.env.VITE_API_URL}/transaction/catUpdate/neutering/${
          ...
          <div className="w-full px-10 flex flex-col justify-center items-center gap-3">
            ...
            <div className="flex w-full my-3 items-center justify-center gap-5">
              <div
                ...
                onClick={() => {
                  setShowNeuteringDialog((prev) => !prev);
                }}
              >
                Cancel
              </div>
...
export default NeuteringDialog;
