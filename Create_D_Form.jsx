...
const Create_D_Form = () => {
  ...
  const formatDate = (dateString) => {
    const parts = dateString.split('/'); // Assuming the input is in MM/DD/YY format
    if (parts.length === 3) {
      const day = parts[1];
      const month = parts[0];
  ...
  const handleSubmit = () => {
    if (
      ...
      setLoading(true);
      const formattedTrapDate = dogForm.isOffline ? formatDate(dogForm.trapDate) : generateCurrentDate();
      const data = {
        ...
        afaTrapDate: dogForm.isOffline ? formattedTrapDate : generateCurrentDate(),
      ...
      axios
        .post(
          ...
          }/api/v2/transaction/sterilization/dog/create`,
          JSON.stringify(data),
          {
            headers: {
  ...
  return (
    <>
      ...
      {showTrapperSuccessDialog && (
        <CustomDialogBox newCaseFileNumber={newCaseFileNumber} />
      )}
