...
const SurgeryDialogCat = ({
  ...
  const handleSubmit = () => {
    if (neuteringData.datePerformed === "") {
      setSnackbarOpen({
        open: true,
        message: "SURGERY DATE NOT FOUND",
        type: "warning",
      });
    } else if (!imgSrc) {
      setSnackbarOpen({
        open: true,
        message: "SURGERY IMAGE NOT FOUND",
        type: "warning",
      });
    } else {
      setLoading(true);
      const data = {
        afaStatusId: statusList.filter((st) => st.StatusName === "surgery")[0]
          .id,
        afaVetId: JSON.parse(localStorage.getItem("AFA_user")).userId,
        afaSurgeryDate: neuteringData.datePerformed,
        afaSurgeryImg: imgSrc,
      };
      console.log(data);
      axios
        .put(
          ...
          }/api/v2/transaction/sterilization/cat/update/surgery/${
...
export default SurgeryDialogCat;
