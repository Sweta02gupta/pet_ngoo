...
const TrappingDialog = ({
  fetchAllTransaction,
  currentModification,
  setShowTrappingDialog,
  statusList,
  ...
  const handleSubmit = () => {
    if (!imgSrc) {
      setSnackbarOpen({
        open: true,
        message: "TRAPPING IMAGE NOT FOUND",
        type: "warning",
      });
    } else {
      setLoading(true);
      const data = {
        TrapDate: generateCurrentDate(),
        TrapImg: imgSrc,
        StatusId: statusList.filter(
          (status) => status.StatusName === "trapped"
        )[0].id,
      };
      console.log(data);
      axios
        .put(
          `${import.meta.env.VITE_API_URL}/transaction/catUpdate/traping/${
          ...
          <div className="w-full px-10 flex flex-col justify-center items-center gap-3">
            ...
            <div className="flex w-full my-3 items-center justify-center gap-5">
              <div
                ...
                onClick={() => {
                  setShowTrappingDialog((prev) => !prev);
                }}
              >
                Cancel
              </div>
